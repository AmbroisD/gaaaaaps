...
import os
import argparse
from scripts.utils import load_json, save_json, get_last_modification
...
def scan_file(file_data, resultdir):
    ...
    tmp_result = load_json(os.path.join(resultdir,
                                        'tmp.json'))
    if os.path.exists(os.path.join(resultdir, file_data[1].split('.D.')[0])):
        ...
        station_dict = {}
        station_dict[file_data[1].split('.D.')[1]] = tmp_result
    save_json(station_dict, file_data[1].split('.D.')[0])
